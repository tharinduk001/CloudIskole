...
export function RegistrationControls({
  ...
  const [registerState, registerAction, registerPending] = useActionState(registerForSession, idleResult);
  const [cancelState, cancelAction, cancelPending] = useActionState(cancelRegistration, idleResult);
    ...
    return (
      <form action={cancelAction} className="flex flex-col gap-2 sm:items-start">
        ...
        <div className="text-success inline-flex items-center gap-1.5 text-sm font-medium">
          ...
          {isOpen ? <span className="text-ink-subtle font-normal">— the join link appears shortly before start</span> : null}
        </div>
        {isOpen ? (
  ...
  if (!isOpen) {
    return <p className="text-ink-muted text-sm">Registration is closed for this session.</p>;
  }
  ...
  return (
    <form action={registerAction} className="flex flex-col gap-2 sm:items-start">
      <input type="hidden" name="sessionId" value={sessionId} />
      <Button type="submit" size="lg" disabled={registerPending} className="w-full sm:w-auto">
        {registerPending ? <Loader2 className="animate-spin" aria-hidden="true" /> : <CalendarCheck aria-hidden="true" />}
        Register
      </Button>
